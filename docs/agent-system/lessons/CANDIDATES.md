...
- proposed_prevention_or_detection: acceptance-relevant pull-request workflows must bind `actions/checkout.with.ref` directly to `${{ github.event.pull_request.head.sha || github.sha }}`, immediately derive the same immutable expected value inside the identity-check step, compare `git rev-parse HEAD` against it with a failure-enforcing exact command shape, and mechanically reject alternate trigger syntax, block-scalar impersonation, suppressed comparisons, or shadowable `AIOS_CANDIDATE_SHA` environment indirection.
