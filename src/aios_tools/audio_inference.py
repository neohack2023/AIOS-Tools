...
class AudioInferenceError(RuntimeError):
    """Fail-closed error raised by the frozen stem inference block."""
...
def run_frozen_stem_inference(
    *,
    source_path: Path,
    model_cache: Path,
    transaction: AudioArtifactTransaction,
) -> dict[str, Any]:
    """Decode, separate, validate, and stage the four frozen UMXHQ stems.

    The function performs no network access and does not promote the transaction.
    """
    ...
    try:
        ...
        raise AudioInferenceError("DETERMINISM_UNAVAILABLE", f"deterministic algorithms could not be enabled: {exc}") from exc
