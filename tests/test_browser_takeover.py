...
def test_takeover_success_requires_post_control_verification():
    async def scenario():
...
def test_post_takeover_verification_required_before_session_available():
    async def scenario():
        purged = []
        checkpoint = UserTakeoverCheckpoint(target_origin=ORIGIN, timeout_seconds=1)
...
def test_takeover_timeout_is_bounded_and_fail_visible():
    async def scenario():
        purged = []
...
def test_takeover_cancel_purges_partial_state():
    async def scenario():
        purged = []
        checkpoint = UserTakeoverCheckpoint(target_origin=ORIGIN, timeout_seconds=1)
...
def test_external_task_cancellation_purges_partial_state_and_propagates():
    async def scenario():
        purged = []
...
def test_sso_cross_origin_does_not_auto_widen_takeover_origins():
    async def scenario():
        purged = []
        checkpoint = UserTakeoverCheckpoint(target_origin=ORIGIN, timeout_seconds=1)
...
def test_explicit_sso_origin_can_be_preadmitted_without_widening_base_origin():
    async def scenario():
...
def test_takeover_resume_proof_origin_must_match_target_origin():
    async def scenario():
        purged = []
        checkpoint = UserTakeoverCheckpoint(
            target_origin=ORIGIN,
...
def test_takeover_checkpoint_has_no_unbounded_debug_mode_switch():
    signature = set(inspect.signature(UserTakeoverCheckpoint.__init__).parameters)
