...
EVALUATOR_VERSION = "execution-trust-binding-harness/0.1.0"
...
def evaluate_trust_binding(binding: dict[str, Any]) -> dict[str, Any]:
    """Evaluate one experimental binding without invoking any executor.

    This function is intentionally independent from the production runner. It
    composes existing evidence into an admission decision but grants no policy,
    capability, consent, credential, or semantic authority.
    """
...
def admit_and_invoke_read_only(
    ...
    *,
    executor: Callable[..., dict[str, Any]] = invoke,
) -> dict[str, Any]:
